  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        {!hasScript && (
          <p className="text-sm text-muted-foreground">Script will appear after the script stage finishes.</p>
        )}
        {scriptError && <p className="text-sm text-[var(--danger)]">{scriptError}</p>}
        {scriptOpen && script && (
          <div className="max-h-80 overflow-y-auto rounded-lg border border-line bg-paper/70 p-4">
      ...
      <div>
        ...
        {hasAudio ? (
          ...
          <p className="text-sm text-muted-foreground">Final audio appears after assembly.</p>
        )}
      </div>
      ...
      <div>
        ...
        {hasVideo ? (
          <video
            controls
            className="w-full overflow-hidden rounded-lg border border-line bg-ink"
            src={videoUrl(jobId)}
            preload="metadata"
          />
        ) : (
